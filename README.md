...
静的HTML制作（デザイン反映）から、同じソースを元にWordPressテーマを生成する開発フローに対応しています。

- **静的ビルド** → `htdocs/` にHTML出力（デザイン確認・静的納品用）
- **WPテーマビルド** → `htdocs_wp/` にPHPテンプレート出力（Docker上のWordPressに直接マウント）
...
### 🐘 WordPressテーマ開発
- **PHPテンプレート自動生成** - AstroページからWordPressテーマを生成（header/footer自動分割）
- **手作りテンプレート優先** - `src/public/wp/` 配下のPHPファイルは自動生成より優先
- **Docker開発環境** - WordPress + MySQL + phpMyAdmin + MailHogをワンコマンド構築
- **固定ページ自動作成** - テーマ内の `page-*.php` から固定ページを自動登録
- **ウォッチビルド** - ソース変更を検知して自動再ビルド + Browser Syncリロード
...
└── public/                 # 静的ファイル
    ...
    ├── favicon.ico         # ファビコン
    └── wp/                 # 🐘 WordPressテーマの手作りテンプレート
        └── wp-content/themes/my-theme/
            ├── functions.php   # テーマ機能（style.css読み込み等）
            ├── style.css       # テーマ宣言
            ├── 404.php / index.php
            └── fn/             # 機能別PHPモジュール

plugins/                    # ビルド用カスタムプラグイン
├── phpOutput.js            # HTML→PHPテンプレート変換
├── themeFiles.js           # 手作りテンプレートの配置制御
├── watchTask.js            # ウォッチビルド + Browser Sync
├── convertWebp.js          # WebP変換
└── imageOptimizer.js       # 画像最適化

docker/                     # Docker環境設定（WordPress開発用）
```

### ビルド出力先

```
htdocs/                     # 静的ビルド出力（npm run build）
└── index.html, _assets/ など

htdocs_wp/                  # WPテーマビルド出力（npm run build:wp）
└── wp/wp-content/themes/{テーマ名}/
    ├── header.php / footer.php   # 自動分割生成
    ├── front-page.php             # index.html から変換
    ├── page-{slug}.php            # サブページから変換
    └── functions.php など         # src/public/wp から配置
```

静的ビルドとWPテーマビルドは出力先が完全に分離されており、互いに影響しません。
...
> パッケージ管理はpnpmに統一しています。lockファイルは `pnpm-lock.yaml` のみをコミットしてください（`package-lock.json` はGit管理外です）。
...
```bash
npm start                # 開発サーバー起動
npm run build            # 静的ビルド（htdocs/に出力）
...
### WordPress開発コマンド
```bash
npm run setup:new        # 新規WordPress環境を構築（起動 + 初期設定）
npm run setup:restore    # ダンプファイルから環境を復元
npm run wp:up            # Dockerコンテナ起動
npm run wp:down          # Dockerコンテナ停止
npm run wp:init          # WordPress初期設定（インストール・テーマ有効化・固定ページ作成）
npm run destroy          # 全データ削除（コンテナ・ボリューム・ビルド成果物）
```
...
```bash
# WordPress開発用設定（抜粋）
SITE_NAME=my_site                          # Dockerコンテナ名のプレフィックス
VITE_THEME_NAME=my-theme                   # テーマ名（ビルド出力先・マウント先に使用）
THEME_PATH=htdocs_wp/wp/wp-content/themes  # テーママウント元（ビルド出力先と一致させる）
STATIC_OUTPUT_DIR=htdocs                   # 静的HTML参照先（固定ページのタイトル取得用）
```
...
### WordPressテーマ
- **出力先**: `htdocs_wp/wp/wp-content/themes/{テーマ名}/` ディレクトリ
- **デプロイ**: テーマディレクトリごと本番のWordPressへアップロード
...
# 2. WordPress環境を構築（初回のみ）
npm run setup:new
# → テーマ有効化・固定ページ自動作成まで完了

# 3. ウォッチビルド開始（src変更 → 自動再ビルド → ブラウザリロード）
npm run watch
```

| URL | 内容 |
|---|---|
| http://localhost:8080 | WordPress（テーマ開発用） |
| http://localhost:8080/wp-admin | 管理画面 |
| http://localhost:3001 | Browser Sync（ウォッチ時のプロキシ） |
| http://localhost:8082 | phpMyAdmin |
| http://localhost:8025 | MailHog（メール確認） |

### テンプレート変換ルール

`build:wp` 実行時、AstroページのHTMLは以下のルールでPHPテンプレートに変換されます。

| ソース | 出力 |
|---|---|
| `src/pages/index.astro` | `front-page.php` |
| `src/pages/{slug}/index.astro` | `page-{slug}.php` |
| `<meta name="wp-template" content="single-news">` を含むページ | `single-news.php`（メタタグ指定が優先） |
| 最初の完全なHTML文書 | `header.php` / `footer.php` に自動分割 |

- アセットパスは `get_template_directory_uri()` ベースに自動書き換えされます
- `style.css` への `<link>` は削除されます（`functions.php` の `wp_head()` 経由で読み込み）

### 手作りテンプレートの優先

`src/public/wp/wp-content/themes/{テーマ名}/` に置いたPHPファイルは、ビルド時にテーマルートへ配置され、**自動生成より優先**されます（同名の自動生成はスキップ）。`functions.php` や `404.php` のほか、自動生成では難しいテンプレートを手書きしたい場合に使用してください。

> このディレクトリは静的ビルド（`npm run build`）では無視され、`htdocs/` には出力されません。

### 固定ページの自動作成

`npm run wp:init`（`setup:new` に含まれる）は、テーマ内の `page-*.php` を検出して同名スラッグの固定ページを自動作成します。ページタイトルは `htdocs/` の静的HTMLの `<h1>` から取得し、取得できない場合はスラッグ名になります。
