import { a as createComponent, b as renderTemplate, c as createAstro, u as unescapeHTML, d as addAttribute, r as renderComponent, g as renderHead, m as maybeRenderHead, F as Fragment } from '../../../../../../../.astro-internal/astro/server.js';
...
const $$Gnavi = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro$1, $$props, $$slots);
  Astro2.self = $$Gnavi;
  const { menu, slug } = Astro2.props;
  return renderTemplate`${maybeRenderHead()}<nav class="c_nav">
  <h2 class="txtHidden">グローバルナビゲーション</h2>
  <!-- <button type="button" class="c_nav_btn"><span>メニューを開く</span></button> -->
  <div class="c_nav_wrapper">
    <ul id="navi">
      ${menu.map((item) => {
    const isActive = item.link === "/" ? slug === "top" : item.link.replace(/\/$/, "").endsWith(`/${slug}`);
    return renderTemplate`<li${addAttribute(item.child ? "has-child" : void 0, "class")}>
              <a${addAttribute(item.link, "href")}${addAttribute(item.blank ? "_blank" : "_self", "target")}${addAttribute(isActive ? "-active" : void 0, "class")}>${unescapeHTML(item.txt)}</a>
              ${item.child && renderTemplate`${renderComponent($$result, "Fragment", Fragment, {}, { "default": ($$result2) => renderTemplate`
                  <button type="button" class="spAccordion">
                    <span class="txtHidden">子メニューを表示します</span>
                  </button>
                  <ul class="subMenu">
                    ${item.child.map((child) => renderTemplate`<li>
                        <a${addAttribute(
      child.anchor && child.link.startsWith("#") ? slug === "sample" ? child.link : `/sample/${child.link}` : child.link,
      "href"
    )}${addAttribute(child.blank ? "_blank" : "_self", "target")}>${unescapeHTML(child.txt)}</a>
                      </li>`)}
                  </ul>
                ` })}`}
            </li>`;
...
}, "/Users/yoshioka.y/project/astro-dev/src/components/Gnavi.astro", void 0);

const $$Astro = createAstro("http://localhost:3000");
const $$Header = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$Header;
  const { slug, siteName, menu } = Astro2.props;
  ...
  <div class="contentInner">
    <div class="header-inner">
      ...
      <!-- グローバルナビゲーション -->
      ${renderComponent($$result, "Gnavi", $$Gnavi, { "menu": menu, "slug": slug })}
    </div>
  </div>
</header>`;
